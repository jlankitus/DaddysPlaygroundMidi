// GearInspector.cs (C)2015 by Alexander Schlottau, Hamburg, Germany
//   shows a custom inspector for the Gear.cs script

using UnityEditor;
using UnityEngine;

[CustomEditor(typeof(ProceduralWormGear))]

public class WormGearInspector : Editor {
	...
	private GUIContent
		menuPrefs    = new GUIContent("Worm Gear", "Basic settings of this gear."),
		...
		radiusContent= new GUIContent("Radius", "The radius of the WormGear"),
		lenghtContent= new GUIContent("Lenght", "The lenght of the WormGear"),
		tContent	 = new GUIContent("Tooth Width", "Thickness of the gear at the base of the teeth."),
		countContent = new GUIContent("Subdivisions", "How many divisios the gear will get.");
	...
	private bool showOptInfo, showMatInfo, showPrefInfo, showUVInfo, symmetric;
	private string path = "Assets";
	private string bSlash {get{ return (Application.platform == RuntimePlatform.OSXEditor) ? "/" : "\\"; }}
	#endregion
	
	[MenuItem("GameObject/Create Other/Procedural Worm Gear", false, 7101)]
	...
	static void CreateNewGear() {
		...
		go.name = "WormGear";
		go.AddComponent(typeof(ProceduralWormGear));
		Selection.activeGameObject = go;
		go.transform.position = position;
		Undo.RegisterCreatedObjectUndo (go, "ProcWormGear Created");
	}
	...
	void OnSceneGUI() {
		...
		ProceduralWormGear gear = (ProceduralWormGear)target;
		...
		for(int i = 0; i < (symmetric ? partCount+1 : gear.points.Length-3); i++) {
			if (i != gear.prefs.bodyParts) {
				...
				Vector3 newPoint = Handles.FreeMoveHandle(oldPoint, Quaternion.identity, handelScale * 0.15f, pointSnap, Handles.DotCap);
	...
	public override void OnInspectorGUI() {
		...
		foreach(ProceduralWormGear g in targets)
	...
	private void MenuPrefs() {
		...
		divisions = prefs.FindPropertyRelative ("divisions"),
		...
		toothWidth = prefs.FindPropertyRelative ("toothWidth"),
		radius = prefs.FindPropertyRelative ("radius"),
		lenght = prefs.FindPropertyRelative ("lenght"),
		lr = prefs.FindPropertyRelative ("lr"),
		basePoint = points.GetArrayElementAtIndex (bodyParts.intValue),
		partCount = basePoint.FindPropertyRelative ("mat");
		if (modul == null || partCount == null)
			return;
		...
		for (int i=0; i<5; i++) {
			...
			if (i < 1)
				EditorGUILayout.PropertyField (modul, modulContent);
			else if (i == 1)
				EditorGUILayout.PropertyField (divisions, countContent);
			else if (i == 2)
				EditorGUILayout.PropertyField (radius, radiusContent);
			else if (i == 3)
				EditorGUILayout.PropertyField (lenght, lenghtContent);
			else
				EditorGUILayout.PropertyField (toothWidth, tContent);
			GUILayout.Space (155);
		...
		if (lr.boolValue) {
						if (GUILayout.Button ("Right winding >", EditorStyles.miniButton))
								lr.boolValue = false;
				} else {
						if (GUILayout.Button ("< Left winding", EditorStyles.miniButton))
								lr.boolValue = true;
				}
		EditorGUILayout.Space ();
		EditorGUILayout.EndHorizontal ();
		...
		if (GUILayout.Button ("Tooth", EditorStyles.miniButton))
			capT.boolValue = !capT.boolValue;
		if (!capB.boolValue)
			GUI.color = new Color (1f, 1f, 1f, 0.6f);
		else
			GUI.color = gCol;
		if (GUILayout.Button ("Body", EditorStyles.miniButton))
			capB.boolValue = !capB.boolValue;
		GUI.color = gCol;
		EditorGUILayout.Space ();
		EditorGUILayout.EndHorizontal ();
	...
	private void MenuMat() {
		...
		for (int i=indexes[0]; i< indexes[1];i++) {
			if (i == points.arraySize-2)  EditorGUILayout.LabelField("Cap Body");
				...
				if (i == points.arraySize-1) EditorGUILayout.LabelField("Cap Tooth");
			SerializedProperty
				point = points.GetArrayElementAtIndex(i),
	...
	private void MenuUV() {
		...
		if (automaticUV.boolValue)
			...
			EditorGUILayout.Separator();
			toothFlanks.boolValue = EditorGUILayout.Toggle("Tooth Caps Planar",toothFlanks.boolValue);
			EditorGUILayout.BeginHorizontal();
			EditorGUILayout.Space();
	...
	private void ShowPrefInfo() {
		...
		EditorGUILayout.Separator();
		GUI.color = Color.white;
		EditorGUILayout.TextArea("Modul:\n\n Modul is the size of the teeth in a norm scale.\n You see two white circles around the gear.\n Between this circles the teeth have to be, to be in norm scale.\n" +
		                         "\nDivisions:\n\n How many divisions on the circumference should be.\n"+
		                         "\nRadius:\n\n This is the radius of the worm gear.\n"+
		                         "\nLenght:\n\n The lenght of the worm gear.\n"+
		                         "\nTooth Width:\n\n How thick the tooth will be.\n Normaly, you set the width so that there is no space between the windings.\n"+
